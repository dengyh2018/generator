...
public abstract class AbstractXmlElementGenerator extends AbstractGenerator {
    ...
    protected XmlElement getBaseColumnListElement(boolean hasBase) {
        XmlElement answer = new XmlElement("include"); //$NON-NLS-1$
        String id;
        if(!hasBase){
            id=introspectedTable.getBaseColumnListId();
        }else{
            id= introspectedTable.getMyBatis3JavaMapperType()+"."+introspectedTable.getBaseColumnListId();
        }
    ...
    protected XmlElement getBlobColumnListElement(boolean hasBase) {
        XmlElement answer = new XmlElement("include"); //$NON-NLS-1$
        ...
        if(!hasBase){
            ...
            id= introspectedTable.getMyBatis3JavaMapperType()+"."+introspectedTable.getBlobColumnListId();
        }
        
        answer.addAttribute(new Attribute("refid", id));
        return answer;
    }
