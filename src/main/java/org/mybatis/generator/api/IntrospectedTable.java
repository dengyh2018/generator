...
import org.mybatis.generator.codegen.mybatis3.PropertiesUtils;
...
public abstract class IntrospectedTable {
    ...
    protected enum InternalAttribute {
        ATTR_DAO_IMPLEMENTATION_TYPE, ATTR_DAO_INTERFACE_TYPE, ATTR_PRIMARY_KEY_TYPE, ATTR_BASE_RECORD_TYPE, ATTR_EXTEND_RECORD_TYPE, ATTR_RECORD_WITH_BLOBS_TYPE, ATTR_EXAMPLE_TYPE, ATTR_IBATIS2_SQL_MAP_PACKAGE, ATTR_IBATIS2_SQL_MAP_FILE_NAME, ATTR_IBATIS2_SQL_MAP_NAMESPACE, ATTR_MYBATIS3_XML_MAPPER_PACKAGE, ATTR_MYBATIS3_EXTEND_XML_MAPPER_PACKAGE, ATTR_MYBATIS3_XML_MAPPER_FILE_NAME, ATTR_MYBATIS3_EXTEND_XML_MAPPER_FILE_NAME,
        /**
         * also
         * used
         * as
    ...
    protected Map<String, List<IntrospectedColumn>> uniqueMap;
    protected TargetRuntime targetRuntime;

    private static final String mybatis3JavaClientSuffix = "Mapper"; //"Dao"
    /**
    ...
    public IntrospectedTable(TargetRuntime targetRuntime) {
        ...
        uniqueMap = new LinkedHashMap<String, List<IntrospectedColumn>>();
    }
     ...
     * <获取扩展类型>
     */
    public String getExtendRecordType() {
        return internalAttributes.get(InternalAttribute.ATTR_EXTEND_RECORD_TYPE);
    }
    ...
    public String getMyBatis3ExtendSqlMapNamespace() {
        String namespace = getMyBatis3ExtendJavaMapperType();
        if (namespace == null) {
            namespace = getMyBatis3FallbackSqlMapNamespace();
        }
    ...
    protected void calculateXmlAttributes() {
        ...
        setMyBatis3ExtendXmlMapperFileName(calculateMyBatis3ExtendXmlMapperFileName());
        setMyBatis3ExtendXmlMapperPackage(calculateExtendSqlMapPackage());
        ...
        setSelectByUniqueStatementId("selectBy"); //$NON-NLS-1$
    }

    private String calculateMyBatis3ExtendXmlMapperFileName() {
        String str = "";
        str += (fullyQualifiedTable.getDomainObjectName());
        str = PropertiesUtils.parseValue(str);
        str += ("Mapper.xml"); //$NON-NLS-1$
        return str;
    }

    private String calculateExtendSqlMapPackage() {
        ...
        String str = PropertiesUtils.parseValue(sb.toString());
    ...
    protected void calculateJavaClientAttributes() {
        ...
        sb.append('.');
        sb.append(fullyQualifiedTable.getDomainObjectName());
        sb.append(mybatis3JavaClientSuffix); // $NON-NLS-1$
        setMyBatis3JavaMapperType(sb.toString());

        // 设置扩展类型
        setMyBatis3ExtendJavaMapperType(PropertiesUtils.parseValue(sb.toString()));
    ...
    protected void calculateModelAttributes() {
        ...
        // 设置扩展类型
        setExtendRecordType(PropertiesUtils.parseValue(sb.toString()));
    ...
    public void setExtendRecordType(String baseRecordType) {
        internalAttributes.put(InternalAttribute.ATTR_EXTEND_RECORD_TYPE, baseRecordType);
    }
    ...
    public String getMyBatis3ExtendXmlMapperPackage() {
        return internalAttributes.get(InternalAttribute.ATTR_MYBATIS3_EXTEND_XML_MAPPER_PACKAGE);
    }
    ...
    public void setMyBatis3ExtendXmlMapperPackage(String mybatis3XmlMapperPackage) {
        internalAttributes.put(InternalAttribute.ATTR_MYBATIS3_EXTEND_XML_MAPPER_PACKAGE, mybatis3XmlMapperPackage);
    }
    ...
    public void setMyBatis3ExtendXmlMapperFileName(String mybatis3XmlMapperFileName) {
        internalAttributes.put(InternalAttribute.ATTR_MYBATIS3_EXTEND_XML_MAPPER_FILE_NAME, mybatis3XmlMapperFileName);
    }
    ...
    public String getMyBatis3ExtendJavaMapperType() {
        return internalAttributes.get(InternalAttribute.ATTR_MYBATIS3_EXTEND_JAVA_MAPPER_TYPE);
    }
    ...
    public void setMyBatis3ExtendJavaMapperType(String mybatis3ExtendJavaMapperType) {
        internalAttributes.put(InternalAttribute.ATTR_MYBATIS3_EXTEND_JAVA_MAPPER_TYPE, mybatis3ExtendJavaMapperType);
    }
    ...
    public void addUniqueColumn(Map<String, List<String>> uniques) {

        for (Entry<String, List<String>> entry : uniques.entrySet()) {
            List<String> columns = entry.getValue();
            boolean isPrimaryKey = true;
            if (columns.size() == primaryKeyColumns.size()) {
                for (String column : columns) {
                    IntrospectedColumn incol = getColumn(column);
                    if (!primaryKeyColumns.contains(incol)) {
                        isPrimaryKey = false;
                        break;
                    }
                }
            ...
            if (!isPrimaryKey) {
                List<IntrospectedColumn> icolumns = new ArrayList<IntrospectedColumn>();
                for (String string : columns) {
                    IntrospectedColumn icolumn = getColumn(string);
                    if (icolumn == null) {
                        throw new RuntimeException("列" + string + "不存在");
                    }
                    icolumns.add(icolumn);
                }
                uniqueMap.put(entry.getKey(), icolumns);
    ...
    public Map<String, List<IntrospectedColumn>> getUniqueMap() {
        return uniqueMap;
    }

    public void setUniqueMap(Map<String, List<IntrospectedColumn>> uniqueMap) {
        this.uniqueMap = uniqueMap;
    }

    public String getSelectByUniqueStatementId() {
        return internalAttributes.get(InternalAttribute.ATTR_SELECT_BY_UNIQUE_STATEMENT_ID);
    }

    public void setSelectByUniqueStatementId(String s) {
        internalAttributes.put(InternalAttribute.ATTR_SELECT_BY_UNIQUE_STATEMENT_ID, s);
    }

    public String getMethodPartName(List<IntrospectedColumn> list) {
        String partName = "";
        for (IntrospectedColumn introspectedColumn : list) {
            partName += upperCaseFirstChar(introspectedColumn.getJavaProperty()) + "And";
        }

        partName = partName.substring(0, partName.lastIndexOf("And"));
        return partName;

    }

    protected static String upperCaseFirstChar(String input) {
