...
public class DatabaseIntrospector {
    ...
    private TableSchemaHelp tableSchemaHelp;
    ...
    public DatabaseIntrospector(Context context,
                                DatabaseMetaData databaseMetaData,
                                TableSchemaHelp tableSchemaHelp,
                                JavaTypeResolver javaTypeResolver, List<String> warnings) {
        super();
        this.context = context;
        this.databaseMetaData = databaseMetaData;
        this.tableSchemaHelp = tableSchemaHelp;
        this.javaTypeResolver = javaTypeResolver;
        this.warnings = warnings;
        logger = LogFactory.getLog(getClass());
    }

    private void calculateTableRemark(FullyQualifiedTable table,
                                      IntrospectedTable introspectedTable) {
//          this.tableSchemaHelp.getTableRemark(tableName);
    ...
    private void calculateUniqueKey(FullyQualifiedTable table, IntrospectedTable introspectedTable) {
        ResultSet rs = null;
        try {
            rs = databaseMetaData.getIndexInfo(table.getIntrospectedCatalog(), table.getIntrospectedSchema(), table.getIntrospectedTableName(), true, false);
            ...
            while (rs.next()) {
                ...
                List<String> columnNames = uniques.get(indexName);
                if (columnNames == null) {
                    columnNames = new ArrayList<String>();
                    uniques.put(indexName, columnNames);
                }
                columnNames.add(columnName);
            }

            introspectedTable.addUniqueColumn(uniques);
    ...
    private Map<ActualTableName, List<IntrospectedColumn>> getColumns(
        ...
        while (rs.next()) {
            ...
            //dengyh
            if ("dbid".equalsIgnoreCase(fieldName)) {
                continue;
            }

            //dengyh
            /*if ("created_by".equalsIgnoreCase(fieldName) || "creation_date".equalsIgnoreCase(fieldName)
                    || "last_updated_by".equalsIgnoreCase(fieldName) || "last_update_date".equalsIgnoreCase(fieldName)
                    || "dbid".equalsIgnoreCase(fieldName)) {
                continue;
            }*/
            ...
            ResultSet tablesi = databaseMetaData.getTables(localCatalog, localSchema,
                    localTableName, null);

            while (tablesi.next()) {
                String tableRemark = tablesi.getString("REMARKS");
            }
    ...
    private List<IntrospectedTable> calculateIntrospectedTables(
        ...
        for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns
            ...
            calculateUniqueKey(table, introspectedTable);
